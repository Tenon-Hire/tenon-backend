...
def test_apply_status_transition_allows_happy_path():
    ...
    changed = trial_service.apply_status_transition(
        sim,
        target_status=trial_service.TRIAL_STATUS_GENERATING,
        changed_at=at,
    )
    assert changed is True
    assert sim.status == trial_service.TRIAL_STATUS_GENERATING
    assert sim.generating_at == at

    changed = trial_service.apply_status_transition(
        sim,
        target_status=trial_service.TRIAL_STATUS_READY_FOR_REVIEW,
        changed_at=at,
    )
    assert changed is True
    assert sim.status == trial_service.TRIAL_STATUS_READY_FOR_REVIEW
    ...
    assert changed is True
    assert sim.status == trial_service.TRIAL_STATUS_ACTIVE_INVITING
