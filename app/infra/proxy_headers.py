...
def _is_valid_cidr(value: str) -> bool:
    try: ipaddress.ip_network(value); return True
    except ValueError: return False
...
class TrustedProxyHeadersMiddleware:
    def __init__(self, app, trusted_proxy_cidrs: list[str] | None = None) -> None:
        ...
        self.app = app
        self._trusted_networks = [ipaddress.ip_network(cidr) for cidr in cidrs if _is_valid_cidr(cidr)]
    async def __call__(self, scope, receive, send):
        ...
        if not _ip_in_trusted(client_host, self._trusted_networks):
            await self.app(scope, receive, send); return
        forwarded_for = dict(scope.get("headers") or {}).get(b"x-forwarded-for", b"").decode("latin1").strip()
        if forwarded_for:
