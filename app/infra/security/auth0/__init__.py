...
def get_jwks() -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("auth0_jwks_fetch_failed", extra={"jwks_url": settings.auth.jwks_url, "reason": "jwks_fetch_failed"})
        raise Auth0Error("Auth provider unavailable", status_code=status.HTTP_503_SERVICE_UNAVAILABLE) from exc
def clear_jwks_cache() -> None:
    with _jwks_lock:
        _jwks_cache["jwks"] = None
        _jwks_cache["fetched_at"] = 0.0

__all__ = ["decode_auth0_token","get_jwks","clear_jwks_cache","_fetch_jwks","_jwks_cache","_jwks_lock","_http_client","httpx","settings","Auth0Error"]
