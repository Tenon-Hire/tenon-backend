...
from .context import PerfStats, clear_request_stats, get_request_stats, start_request_stats
...
def attach_sqlalchemy_listeners(engine) -> None:
    ...
    if _listeners_attached:
        return
    register_listeners(engine, event_impl=event, perf_ctx=_perf_ctx, perf_module=sys.modules[__name__])
    _listeners_attached = True

__all__ = ["PerfStats","RequestPerfMiddleware","attach_sqlalchemy_listeners","perf_logging_enabled","_perf_ctx","_start_request_stats","_get_request_stats","_clear_request_stats","_request_id_from_scope","_listeners_attached","event","settings"]
